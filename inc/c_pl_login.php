...
class PL_Login
{
    ...
    private function loginWithPostData()
    {
        ...
        if (!empty($_POST['user_name']) && !empty($_POST['user_password'])) {
            ...
            $sqlinf = Config::getMysqlInfo();
            $usertable = $sqlinf['user_table'];
            
            // create a database connection, using the constants from config/db.php (which we loaded in index.php)
            $this->db_connection = new mysqli($sqlinf['host'], $sqlinf['user'], $sqlinf['pass'], $sqlinf['db']);
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $checklogin = $this->db_connection->query("SELECT user_name, user_email, user_password_hash FROM $usertable WHERE user_name = '" . $this->user_name . "';");
                if($checklogin){
                // if this user exists
